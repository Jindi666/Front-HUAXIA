<template>
  <div class="chart-container2">
    ...
    <div class="empty-mask" :style="{ visibility: maskVisible }">
      <!-- <h2 class="title1">{{ $t('dashboard.digitalEmployeeBusinessVolume.text') }}</h2> -->
...
export default {
  name: 'DigitalEmployeeBusinessVolume',
  props: {
    refreshInterval: {
      type: Number,
      default: function() {
  ...
  methods: {
    getOption() {
      let option = {
        ...
        title: {
          // text: this.$t('dashboard.digitalEmployeeBusinessVolume.text'),
          left: 0,
          top: 12,
          textStyle: {
            color: '#8AABFB'
    ...
    loadData(type) {
      ...
      this.getApi()
        .queries(queryId.QUERYSCREEN_BARCHART_TOP10_STAFFNAME_BUSINESSVOLUME)
        .then(response => {
