<template>
  <div class="gb-ant-select-multiple-cascader" :class="{ 'multiple-cascader-outside': !isClickOutSide }" v-clickoutside="handleDropDownOutSide">
    <a-select
      v-bind="$attrs"
      v-on="$listeners"
      mode="multiple"
      ...
      @onDeselect="handleDeselect2"
    >
      <a-icon type="close-circle" class="c-icon" slot="clearIcon" theme="filled" @click.stop="handleClearAll" />
        ...
        <div :class="dropdownClassName">
          ...
          <div class="cascader-content-wrap" v-else>
            <div class="cascader-content-container">
              <ul class="cascader-content-list" v-for="(itemList, levelIndex) in treeDataList" :key="levelIndex">
                <!-- <li class="cascader-content-item cascader-content-item-check-all" @click="handleCascaderAllItemClick(isCheckAll)" v-if="levelIndex === 0">
                  <a-checkbox :indeterminate="isIndeterminateAll" v-model="isCheckAll" />
                ...
                <li
                  ...
                  @click="handleCascaderItemClick(subItem, subIndex, levelIndex)"
                >
                  <a-checkbox :indeterminate="!!subItem.$indeterminate" :checked="!!subItem.$checked" @click.self="e => handleCheckClick(e, subItem, subIndex, levelIndex)" />
                  <span class="checkbox-text">{{ subItem[selectOptionsConfig.text] }}</span>
                  <a-icon type="right" style="font-size:10px;" v-if="subItem[selectOptionsConfig.children] && subItem[selectOptionsConfig.children].length > 0" />
                </li>
              </ul>
            </div>
          </div>
      ...
      <a-select-option v-for="item in treeToList" :key="item[selectOptionsConfig.key]" :value="item[selectOptionsConfig.value]" :text="item[selectOptionsConfig.text]">
...
export default {
  ...
  methods: {
    ...
    initDefaultKeys(selectedArr, treeDataList) {
      if (selectedArr && selectedArr.length > 0 && treeDataList && treeDataList.length > 0) {
        ...
        selectedArr.forEach(item => {
          ...
          if (!treeToListClone.find(listItem => listItem[this.selectOptionsConfig.value] === currentItem[this.selectOptionsConfig.value])) {
            treeToListClone.push(currentItem)
          }
          currentItem.$checked = true
          if (currentItem[this.selectOptionsConfig.children] && currentItem[this.selectOptionsConfig.children].length > 0) {
            this.setAllDataTreeChecked(currentItem[this.selectOptionsConfig.children], true)
          }
        })
    ...
    handleDeselect(value) {
      ...
      if (currentItem[this.selectOptionsConfig.children] && currentItem[this.selectOptionsConfig.children].length > 0) {
        this.setAllDataTreeChecked(currentItem[this.selectOptionsConfig.children], false)
      }
      this.setCheckBoxIndeteriminate(treeDataListClone[0])
      this.treeDataList = treeDataListClone
      this.updatePostData()
    ...
    handleCascaderItemClick(item, index, levelIndex) {
      ...
      this.findAllParentItems(currentItem[valueTag], treeDataListClone[0], treeDataListClone[0], parentItemsList)
    ...
    handleDeselect2() {
      this.$emit('deselect', this.selectedValueArr)
    },
    updatePostData() {
      const childrenTag = this.selectOptionsConfig.children
      const valueTag = this.selectOptionsConfig.value
      ...
      function _getAllItemChecked(treeDataList) {
        ...
        treeDataList.forEach(item => {
          if (item[childrenTag] && item[childrenTag].length > 0) {
            ...
            } else if (currentCheckedAll.some(checkItem => checkItem === true)) {
      ...
      function _getAllItemChecked(treeDataList) {
        ...
        treeDataList.forEach(item => {
          if (item[childrenTag] && item[childrenTag].length > 0) {
            ...
            } else if (currentCheckedAll.every(checkItem => checkItem === false)) {
              parentItem.$checked = false
              parentItem.$indeterminate = false
            }
          }
    ...
    findAllParentItems(currentKey, treeDataList, originTreeDataList, parentItemsList, parentItem, parentItemsHasKey) {
      if (parentItem && parentItemsHasKey) {
        parentItemsList.unshift(parentItem)
        this.findAllParentItems(parentItem[this.selectOptionsConfig.value], treeDataList, originTreeDataList, parentItemsList)
      } else {
        this.findKeysAndParentItem(currentKey, treeDataList, originTreeDataList, parentItemsList, parentItem, parentItemsHasKey)
      }
    },
    findKeysAndParentItem(currentKey, treeDataList, originTreeDataList, parentItemsList, parentItem) {
      const childrenTag = this.selectOptionsConfig.children
      treeDataList.forEach(item => {
        if (item[this.selectOptionsConfig.value] === currentKey && parentItem) {
          this.findAllParentItems(parentItem[this.selectOptionsConfig.value], originTreeDataList, originTreeDataList, parentItemsList, parentItem, true)
        } else {
          if (item[childrenTag] && item[childrenTag].length > 0) {
            this.findKeysAndParentItem(currentKey, item[childrenTag], originTreeDataList, parentItemsList, item)
          }
        }
      })
    },
    ...
    addHideSelectedList(treeList) {
      ...
      treeList.forEach(item => {
        if (!treeToListClone.find(listItem => listItem[this.selectOptionsConfig.value] === item[this.selectOptionsConfig.value])) {
