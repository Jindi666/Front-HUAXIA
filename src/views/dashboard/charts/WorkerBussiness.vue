<template>
  <div class="chart-container2">
    ...
    <div class="option-box">
      ...
      <strong>{{ $t('dashboard.bussinessTypeDistribute.text') }}</strong>
    </div>
  </div>
</template>
  ...
  methods: {
    getOption() {
      ...
      return {
        ...
        series: [
          {
            name: this.$t('dashboard.businessType.text'),
            type: 'pie',
            radius: '55%',
            center: ['50%', '50%'],
            data: [],
    ...
    initChart() {
      ...
      this.chart.on('click', params => {
        ...
        this.$router.push({
          ...
          query: { bussinessType: params.data.key, cycle: this.cycle }
        })
      })
    },
    loadData(type) {
      this.chart.showLoading()
      dashboardApi.queryBusinessType(type).then(response => {
