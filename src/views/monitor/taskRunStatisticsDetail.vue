<template>
  <div id="taskRunStatisticsDetail">
...
export default {
  data() {
    const columns = [
      ...
      },
      {
        title: '运行中任务',
        width: 160,
        dataIndex: 'taskRunning'
      },
      {
        title: '待运行任务',
        width: 160,
        dataIndex: 'taskDeploying'
      }
    ]
    return {
      loading: false,
  ...
  mounted() {
    document.title = '任务运行统计——详细列表'
    this.getTaskRunStatisticsPage()
  },
  methods: {
    ...
    getTaskRunStatisticsPage() {
      this.loading = true
      Api.getTaskRunStatisticsPage(this.page).then(response => {
        this.tableData = []
        this.loading = false
